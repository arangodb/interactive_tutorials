...
run_import () {
  ...
  local type="$3"
  shift 3
  $ARANGOIMPORT --type csv --collection "$collection" --separator "|" --create-collection true --create-collection-type "$type" --file "$file" --server.endpoint "$ARANGO_ENDPOINT" --server.database "$ARANGO_DATABASE" --server.username "$ARANGO_USERNAME" --server.password "$ARANGO_PASSWORD" "$@"
  import_result=$?
}
...
process_directory "dynamic/Person_hasInterest_Tag" "Person_hasInterest_Tag" "edge" "--datatype personId=string" "--datatype TagId=string" "--from-collection-prefix=Person" "--to-collection-prefix=Tag" "--merge-attributes _from=[personId]:[personId]" "--merge-attributes _to=[TagId]"
